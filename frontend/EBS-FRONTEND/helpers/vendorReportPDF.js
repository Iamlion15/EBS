...
const FinanceGeneratePDF = (dataa, pData) => {
  const unit = 'mm';
  const size = 'a4';
  ...
  const printData = dataa.map((data, index) => {
    ...
    const payedOn=formatDateToCustomFormat(data.Finance_Approval.timeOfApproval)

    return [index + 1, itemName, names, requestedOn,payedOn];
  });

  // Auto-generate a table in the PDF using autoTable
  doc.autoTable({
    head: [['No.', 'ITEM NAME', 'STAFF NAME', 'REQUESTED ON','PAYED ON']],
    body: printData,
    startY: startY,
...
export default FinanceGeneratePDF;
